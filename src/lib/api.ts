...
export async function apiFetch(path: string, init?: RequestInit): Promise<Response> {
  ...
  if (!requestCredential) throw new Error("operator_credential_required");
  const requestToken = requestCredential.token;
  suppliedHeaders.set("Authorization", `Bearer ${requestToken}`);
